                        ...
                        try:
                            ...
                            if validation['valid']:
                                ...
                                if (xau_signal_data.get('signal') and
                                    ...
                                    logger(f"🥇 XAU/USD OPTIMIZED: {xau_signal_data['signal']} - {xau_signal_data['confidence']:.1%} confidence")
                                    logger(f"   📊 TP: {xau_signal_data['tp_pips']} pips, SL: {xau_signal_data['sl_pips']} pips")
                                    logger(f"   📈 Position multiplier: {xau_signal_data['position_size_multiplier']:.1f}x")
                                    ...
                                    logger(f"⚠️ XAU/USD OPTIMIZER: Signal rejected - {xau_signal_data.get('reason', 'Low confidence')}")
                            else:
                                logger(f"⚠️ XAU/USD CONDITIONS: Not optimal - {validation.get('reason', 'Conditions not met')}")
                                logger(f"   📊 Spread: {validation.get('spread_usd', 0):.1f} USD | Session: {validation.get('optimal_session', False)} | Volatility: {validation.get('volatility_ok', False)}")

                        except Exception as xau_opt_e:
                            logger(f"⚠️ XAU/USD optimization error: {str(xau_opt_e)}")
